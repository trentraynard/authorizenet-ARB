...
/**
 *
 * @author VM Team Modified by Trent Raynard
...
if (!class_exists('AuthnetARB')){
	require(JPATH_ROOT . '/plugins/vmpayment/authorizenetrecurring/authorizenet/fields/authnetARB.php');
}

if (!class_exists('VmConfig')) {
	require(JPATH_VM_ADMINISTRATOR . '/helpers/config.php');
}

class plgVmpaymentAuthorizenetRecurring extends vmPSPlugin {
	...
	//Pull flag value from checkout page
...
//FITLER CUSTOM VARIABLES HERE TO FLAG SUBSCIPTION ITEM($subscriptionValue) TO SEND TO Recurring Payment FUNC. ADD ONE MONTH EXTRA TO START DATE AND SO THEY ARE CHARGED LIKE NORMAL ONE-TIME THEN ARB STARTS ONE MONTH LATER		
...
//to do create correct rows to store in backend
	...
	/*setting up recurring payment using authernetARB
	API calls from authnetARB class.
	REMOVE 3rd class parameter when going live
	*/
	function recurringPayment($amt) {
  try
  { 

	$subscription = new AuthnetARB($this->_getLoginId(), $this->_getTransactionKey(), AuthnetARB::USE_DEVELOPMENT_SERVER);
	// subscription parameters to pass -- get from cart and AMOUNT OF SUB PRICE TOTAL TO BE CHARGED
	
	$subscription->setParameter('amount', int);
	
	$subscription->setParameter('cardNumber', $this->_cc_number = str_replace(" ", "", vRequest::getVar('cc_number_' . $cart->virtuemart_paymentmethod_id, '')));
	
	$subscription->setParameter('expirationDate', $this->_getFormattedDate($this->_cc_expire_month, $this->_cc_expire_year));

	$subscription->setParameter('firstName', (isset($usrBT->first_name)) ? $this->_getField($usrBT->first_name, 50) : '');

	$subscription->setParameter('lastName', (isset($usrBT->last_name) )? $this->_getField($usrBT->last_name, 50) : '');

	$subscription->setParameter('address', (isset($usrBT->address_1) ) ? $this->_getField($usrBT->address_1, 60) : '');
	
	$subscription->setParameter('city', (isset($usrBT->city) ) ? $this->_getField($usrBT->city, 40) : '');
	
	$subscription->setParameter('state', (isset($usrBT->virtuemart_state_id) ) ? $this->_getField(ShopFunctions::getStateByID($usrBT->virtuemart_state_id), 40) : '');
	
	$subscription->setParameter('zip', (isset($usrBT->zip) ) ? $this->_getField($usrBT->zip, 20) : '');
	
	$subscription->setParameter('email', (isset($usrBT->email) ) ? $this->_getField($usrBT->email, 100) : '');
 

 	/*  three months subsciption
	...
	*/
	...
	{
    // alert thst the subscription was not created!
	}

	//TO DO ---save subscribtionID into database table with customer info of ARB
...
/* use delete subscribtion accounts
 need the correct parameters to call
----subscriptionid from database of account to cancel
 */

 	function _deleteSubAccount()
 	{
 	$subscription = new AuthnetARB('myapilogin', 'myTransnKey');
